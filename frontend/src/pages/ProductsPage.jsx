...
const ProductCard = ({ product, onCart }) => {
  ...
  return (
    <div className="bg-white d-flex flex-column" style={{ border: "1px solid #e5e7eb" }}>
      ...
      <div className="p-3 flex-grow-1 d-flex flex-column gap-1">
        ...
        {product.category === "book" && product.author && (
          <div className="text-muted" style={{ fontSize: "0.78rem" }}>by {product.author}</div>
        )}
        <div className="fw-bold mt-auto" style={{ fontSize: "1.05rem" }}>₹{product.price}</div>
        <div className="d-flex align-items-center gap-2">
          <span className={`fw-bold ${inStock ? "text-success" : "text-danger"}`} style={{ fontSize: "0.72rem" }}>
            {inStock ? "In Stock" : "Out of Stock"}
          </span>
          {inStock && <span className="text-muted" style={{ fontSize: "0.72rem" }}>{product.stock_quantity || product.stock} available</span>}
        </div>
        {product.description && (
        ...
        <button onClick={() => onCart(product.id)} disabled={!inStock}
          className={`btn btn-sm fw-bold mt-1 d-flex align-items-center justify-content-center gap-1 ${inStock ? "btn-dark" : "btn-light text-muted"}`}>
        ...
        <div className="bg-light border rounded-3 p-3 mb-4">
          <div className="d-flex gap-2 flex-wrap align-items-center mb-2">
            ...
            <div className="input-group flex-grow-1" style={{ minWidth: 200, maxWidth: 360 }}>
              ...
              <button onClick={() => fetchProducts(1)} className="btn btn-dark fw-bold">Search</button>
            </div>

            <select value={category} onChange={e => { setCategory(e.target.value); setTimeout(() => fetchProducts(1), 0); }} className="form-select" style={{ width: "auto" }}>
            ...
            <select value={inStock} onChange={e => { setInStock(e.target.value); setTimeout(() => fetchProducts(1), 0); }} className="form-select" style={{ width: "auto" }}>
              <option value="all">All Stock</option>
              <option value="inStock">In Stock</option>
              <option value="outOfStock">Out of Stock</option>
            </select>

            <select value={sortBy} onChange={e => { setSortBy(e.target.value); setTimeout(() => fetchProducts(1), 0); }} className="form-select" style={{ width: "auto" }}>
