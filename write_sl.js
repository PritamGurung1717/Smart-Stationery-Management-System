  ...
  <div style={{ position: "fixed", inset: 0, zIndex: 2000 }}>
    ...
    <div style={{ position: "absolute", right: 0, top: 0, bottom: 0, width: 380, background: "#fff", boxShadow: "-4px 0 24px rgba(0,0,0,0.1)", display: "flex", flexDirection: "column" }}>
      <div style={{ padding: "1.25rem 1.5rem", borderBottom: "1px solid #e5e7eb", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h3 style={{ margin: 0, fontWeight: 700, fontSize: "1.05rem" }}>Wishlist ({wishlist.length})</h3>
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: "1rem 1.5rem" }}>
        {wishlist.length === 0 ? (
          ...
          <div key={item.id || item.product_id} style={{ display: "flex", gap: "0.75rem", padding: "0.75rem 0", borderBottom: "1px solid #f3f4f6" }}>
...
export const Navbar = ({ activeLink = "" }) => {
  ...
  useEffect(() => {
    ...
    axios.get(\`\${API}/wishlist\`, { headers: authH() })
      .then(r => {
        if (r.data.success) {
          setWishlist(r.data.wishlist.map(i => ({ ...i.product, wishlistId: i._id, product_id: i.product?._id })));
        }
      }).catch(() => {});
  }, []);
      ...
      <nav style={{ position: "sticky", top: 0, zIndex: 1000, background: "#fff", borderBottom: "1px solid #e5e7eb" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "0 1.5rem", display: "flex", alignItems: "center", justifyContent: "space-between", height: 56 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "1.1rem" }}>
            ...
            <NotificationBell />
            <button onClick={() => setShowWishlist(true)} style={{ background: "none", border: "none", cursor: "pointer", color: "#111", fontSize: "1.05rem", position: "relative" }}>
              <FaHeart />
              {wishlist.length > 0 && (
...
fs.writeFileSync('frontend/src/components/SharedLayout.jsx', content, 'utf8');
